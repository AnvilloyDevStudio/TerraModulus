		...
		switch (mob) {
			case "Air Wizard":
				...
				if (level.depth == 1) {
					...
					Game.inGameNotifications.add(Localization.getStaticDisplay("minicraft.notification.wrong_level_sky"));
				}
					...
					if (new Rectangle(level.w / 2 - 3, level.h / 2 - 3, 7, 7).contains(player.x >> 4, player.y >> 4)) {
						if (!ObsidianKnight.active) {
							...
							if (!exists) { // Prevent unintended behaviors
								...
								Game.inGameNotifications.add(Localization.getStaticDisplay("minicraft.notification.knight_statue_exists"));
							}
						} else {
							Game.inGameNotifications.add(Localization.getStaticDisplay("minicraft.notification.boss_limit"));
						}
					} else {
						Game.inGameNotifications.add(Localization.getStaticDisplay("minicraft.notification.spawn_on_boss_tile"));
					}
				} else {
					Game.inGameNotifications.add(Localization.getStaticDisplay("minicraft.notification.wrong_level_dungeon"));
				}
				break;
			default:
