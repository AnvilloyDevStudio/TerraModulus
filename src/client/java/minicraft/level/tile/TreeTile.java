...
public class TreeTile extends Tile {
	...
	public void render(Screen screen, Level level, int x, int y) {
		...
		if (isUpTileSame && isUpLeftTileSame && isLeftTileSame) {
			screen.render(null, (x << 4) + 0, (y << 4) + 0, spriteFull.spritePixels[0][1]);
		} else {
			screen.render(null, (x << 4) + 0, (y << 4) + 0, sprite.spritePixels[0][0]);
		}

		if (isUpTileSame && isUpRightTileSame && isRightTileSame) {
			screen.render(null, (x << 4) + 8, (y << 4) + 0, spriteFull.spritePixels[0][0]);
		} else {
			screen.render(null, (x << 4) + 8, (y << 4) + 0, sprite.spritePixels[0][1]);
		}

		if (isDownTileSame && isDownLeftTileSame && isLeftTileSame) {
			screen.render(null, (x << 4) + 0, (y << 4) + 8, spriteFull.spritePixels[1][1]);
		} else {
			screen.render(null, (x << 4) + 0, (y << 4) + 8, sprite.spritePixels[1][0]);
		}

		if (isDownTileSame && isDownRightTileSame && isRightTileSame) {
			screen.render(null, (x << 4) + 8, (y << 4) + 8, spriteFull.spritePixels[1][0]);
		} else {
			screen.render(null, (x << 4) + 8, (y << 4) + 8, sprite.spritePixels[1][1]);
		}
	}
