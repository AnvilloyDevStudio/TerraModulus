	...
	public void render(Screen screen) {
		for (int x = 0; x < 200; x++) { // Loop however many times depending on the width (It's divided by 3 because the pixels are scaled up by 3)
			for (int y = 0; y < 150; y++) { // Loop however many times depending on the height (It's divided by 3 because the pixels are scaled up by 3)
				...
				if (dd < 0 && dd > -30) {
					if (dir > 0)
						screen.render(null, x * 8, y * 8, 5, 2, 0, hudSheet.get()); // If the direction is upwards then render the squares going up
					else
						screen.render(null, x * 8, Screen.h - y * 8 - 8, 5, 2, 0, hudSheet.get());  // If the direction is negative, then the squares will go down.
				}
			}
		}
	}
